...
static void formalsTo3AC(Procedure * proc,
  std::list<FormalDeclNode *> * myFormals){
	...
	for (auto formal : *myFormals){
		...
		Quad * inQuad = new GetArgQuad(argIdx, opd, false, myFormals->size());
		proc->addQuad(inQuad);
		argIdx += 1;
	}
}
...
*/
	...
	AssignQuad * quad;
	quad = new AssignQuad(lhs, rhs, false);
...
Opd * MagicNode::flatten(Procedure * proc){
	Opd * resVal = proc->makeTmp(8);
	proc->addQuad(new MagicQuad(resVal));
	return resVal;
}
